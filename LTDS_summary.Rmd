...
n.occ <- nrow(yes.data)
all.data <- as.data.frame(cbind(n.occ, params$effort, params$tort.pop, params$tort.pop.lo, params$tort.pop.hi, cv.new, params$tort.dens,
                                params$tort.dens.lo, params$tort.dens.hi))
colnames(all.data) <- c("N.occ", "Effort (m)", "D", "D UCI", "D LCI", "CV (%)", "N", "N UCI", "N LCI")
#Create table
all.data %>%
  gt() %>%
...
```{r occ.burr.dem, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
ltds_data <- read_csv("./LTDS_example_data.csv")
...
```{r occ.summ.table, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
ltds_data <- read_csv("./LTDS_example_data.csv")
...
```{r det.map, echo=FALSE, warning=FALSE, message=FALSE, verbose = FALSE, error=FALSE}
...
```{r insert.plot, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
...
```{r veg.map, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE, verbose = FALSE}
...
dfr <- rasterFromXYZ(new.veg, crs = "+proj=utm +zone=17 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0")

test <- aggregate(dfr, fact = 100, fun=mean, expand=FALSE)
test <- disaggregate(test, 100)
