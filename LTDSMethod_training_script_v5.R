...
#We need a specific version of one of our pakages (raster), we can install this
#version using the following two lines:
...
#After installation, manually check the package version --->
...
area <- sum(site@data@values, na.rm = TRUE)
(sqrt(area)*5)^2/4047
...
veg.loc <- veg_plot(pilot.lines, intv = 75, save = T, plot = F)
...
#Re-run the veg_plot function
veg.loc <- veg_plot(full.ltds, intv = 75, save = T, plot = F)
...
#setwd("~/LTDS/R_packages/LTDS_R_package/LTDSMethod/vignettes/")
setwd("C:/Users/maxdoltonjones/Desktop/LTDSMethod")

#Move the following files into the "vignettes" folder of the directory
#1. Pilot transect lines
#2. Full transect lines
#3. Site shapefile
#4. LTDS data
#5. Other observation data
